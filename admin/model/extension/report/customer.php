...
class ModelExtensionReportCustomer extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT c.customer_id, c.fullname AS customer, c.email, cgd.name AS customer_group, c.status, o.order_id, SUM(op.quantity) as products, o.total AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE o.customer_id > 0 AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND c.fullname LIKE '" . $this->db->escape((string)$data['filter_customer']) . "'";
		}
	...
	public function getTotalRewardPoints($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "c.fullname LIKE '" . $this->db->escape((string)$data['filter_customer']) . "'";
		}
	...
	public function getCustomerSearches($data = array()) {
		$sql = "SELECT cs.customer_id, cs.keyword, cs.category_id, cs.products, cs.ip, cs.date_added, c.fullname AS customer FROM " . DB_PREFIX . "customer_search cs LEFT JOIN " . DB_PREFIX . "customer c ON (cs.customer_id = c.customer_id)";
